...
An [assignment expression](./assignment) defines the value written by an
accepted update. `setUpdateWhere` decides whether that update is eligible.
It remains effective even when an [upsert mask](./upsert-mask) selects no ordinary
update assignments.
...
or [pessimistic locking](./pessimistic-locking) when concurrent changes must be excluded.

See [Save Result Fetching](./returning#acceptance) for the meaning of `isAccepted`.
