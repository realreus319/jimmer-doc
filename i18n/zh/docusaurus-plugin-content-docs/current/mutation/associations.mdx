...
要插入由类型安全查询提供的大量中间表关联对，使用[从查询插入和合并数据](./insert-from-select)。
关联插入支持忽略冲突，以及返回新插入的关联对。Java接受`AssociationTable`，Kotlin提供关联属性重载。

同一个Java关联表也可以传入`createQuery`和`createBaseQuery`。
`createAssociationQuery`作为已弃用的查询别名保留。
